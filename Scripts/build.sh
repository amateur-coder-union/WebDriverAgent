...
function define_xc_macros_tvos() {
  case "$TARGET" in
    "lib" ) XC_TARGET="WebDriverAgentLib-tvOS";;
    "runner" ) XC_TARGET="WebDriverAgentRunner-tvOS";;
    *) echo "Unknown TARGET"; exit 1 ;;
  esac
  ...
  case "$ACTION" in
    ...
    "analyze" )
      ...
      XC_MACROS="${XC_MACROS} CLANG_ANALYZER_OUTPUT=plist-html CLANG_ANALYZER_OUTPUT_DIR=\"$(pwd)/clang\""
    ;;
    "unit_test" ) XC_ACTION="test -only-testing:UnitTests-tvOS";;
    "int_test_1" ) XC_ACTION="test -only-testing:IntegrationTests_1-tvOS";;
    "int_test_2" ) XC_ACTION="test -only-testing:IntegrationTests_2-tvOS";;
    *) echo "Unknown ACTION"; exit 1 ;;
  esac
